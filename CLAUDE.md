...
- **Lancer l'éval** : `cd backend && python eval/run_eval.py [--judge] [--mmr-threshold X] [--inject-threshold Y]` (requiert `BACKEND_URL` et `EVAL_JWT`)
...
5. **Cross-encoder** (`_rerank_chunks`) sur tout le pool → `rerank_score`, **logit brut** (~-11 à +11). La paire est `(question, « nom [date]\n texte »)` — le nom de la source en fait partie.
6. **Score final** — `0.7 × sigmoid(rerank_score) + 0.3 × score_temporel`. Le `decay` passe de 180 à 30 jours si la question contient un mot temporel.
...
9. **Injection** — seuil sur le **logit brut** (-1.0, ou -2.0 pour une question de type compte-rendu), `MAX_INJECT` 6 ou 8.

### Pièges vérifiés en production — ne pas les réintroduire

- **Les trois étages de recherche doivent voir le nom du fichier.** L'embedding via le préfixe d'`index_source`, le FTS via le tsvector, le cross-encoder via sa paire. Chaque fois qu'un seul l'a manqué, un document nommé explicitement dans la question est devenu introuvable — c'est arrivé deux fois de suite en août 2026, sur le FTS puis sur le reranker.
